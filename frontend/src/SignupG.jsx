...
function SignupG() {
  ...
  const [testOpinion, settestOpinion] = useState(""); // Set default value to 'Test Opinion'
  const [overallOpinion, setOverallOpinion] = useState("Test Opinion"); // Set default value to 'Test Opinion'
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .post("http://localhost:3001/registerCollegeG", {
        name,
        username,
        password,
        email,
        ...
        major,
        graduationYear,
        opinion: [academicOpinion, nonAcademicOpinion, testOpinion, overallOpinion],
      })
      .then((result) => {
        console.log(result);
  ...
  <input
    ...
    name="testOpinion"
    className="form-control rounded-0"
    onChange={(e) => settestOpinion(e.target.value)}
  />
</div>
          <div>
            <label htmlFor="overallOpinion">
